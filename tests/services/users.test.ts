...
import {FailedToLogin, NotUniqueEmail, NotUniqueUsername} from "../../src/helpers/exceptions";
...
describe("login", async () => {
    ...
    it("raises error on password mismatch", async () => {
        ...
        assertError(
            login(
                {email: user.email, password: userData.password + "invalid"},
                {
                    ip: faker.internet.ipv4(),
                    userAgent: faker.internet.userAgent(),
                }
    ...
    it("raises error on email mismatch", async () => {
        ...
        assertError(
            login(
                {email: "invalid" + user.email, password: userData.password},
                {
                    ip: faker.internet.ipv4(),
                    userAgent: faker.internet.userAgent(),
                }
    ...
    it("raises error on email and password mismatch", async () => {
        ...
        assertError(
            login(
                {email: "invalid" + user.email, password: userData.password + "invalid"},
                {
                    ip: faker.internet.ipv4(),
                    userAgent: faker.internet.userAgent(),
                }
